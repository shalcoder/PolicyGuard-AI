...
class PolicyStorage:
        ...
        try:
            if not firebase_admin._apps:
                ...
                if creds_input.strip().startswith('{'):
                    ...
                    raise FileNotFoundError(f"Firebase Credentials not found at {creds_input}")
    ...
    def add_policy_vectors(self, policy_id: str, chunks: List[str], vectors: List[List[float]]):
        ...
        for i, (chunk, vec) in enumerate(zip(chunks, vectors)):
            entry = {"policy_id": policy_id, "chunk_id": f"{policy_id}_{i}", "chunk_text": chunk, "vector": vec}
    ...
    def search_relevant_policies(self, query_vec: List[float], top_k: int = 5) -> List[dict]:
        ...
        for item in self._vector_store:
            ...
            sim = np.dot(q_vec, doc_vec) / (q_norm * d_norm) if d_norm != 0 else 0
    ...
    def add_evaluation(self, report: dict):
        import datetime
        record = {"timestamp": datetime.datetime.now().isoformat(), "report": report}
    ...
    def get_dashboard_stats(self):
        ...
        for entry in self._evaluations:
            risk = entry.get('report', {}).get('risk_assessment', {})
        ...
        for entry in self._evaluations:
            rating = entry.get('report', {}).get('risk_assessment', {}).get('overall_rating', 'Low')
            if rating in risk_counts: risk_counts[rating] += 1
            else: risk_counts["Low"] += 1 
        import datetime, random
        today = datetime.date.today()
        trends = []
        base_score = 100 if total_evaluations == 0 else 85
        for i in range(6, -1, -1):
            date_label = (today - datetime.timedelta(days=i)).strftime("%b %d")
            daily_score = base_score + random.randint(-5, 5)
            if i == 0: daily_score = 100 if violations == 0 else 65
            trends.append({"date": date_label, "score": min(100, max(0, daily_score))})
        return {
            "traces_analyzed": total_evaluations, "violations": violations,
            "active_policies": active_policies, "system_health": 100 if violations == 0 else 65, 
            "risk_distribution": [{"name": "High", "value": risk_counts["High"]}, {"name": "Medium", "value": risk_counts["Medium"]}, {"name": "Low", "value": risk_counts["Low"]}],
            "compliance_trend": trends, "recent_evaluations": recent, "top_business_risks": self._get_business_risks()
        }

    def _get_business_risks(self):
        if not self._evaluations: return None
        ...
        if latest_high_risk:
            ...
            return {"financial": impact.get('financial_exposure', 'Low'), "financial_cost": impact.get('estimated_cost', '$0'), "regulatory": impact.get('regulatory_penalty', 'None'), "brand": impact.get('brand_reputation', 'Stable')}
        return None
    ...
    def get_monitor_data(self):
        ...
        for idx, entry in enumerate(reversed(sorted_evals[-20:])):
            ...
            status = 'block' if rating == 'High' else 'warn' if rating == 'Medium' else 'pass'
            traces.append({
                "id": f"TR-{1000 + idx}",
                ...
                "action": "Policy Scan", "status": status, "details": verdict.get('status_label', 'Evaluated')
            })
        return {"traces_per_min": traces_per_min, "blocking_rate": blocking_rate, "active_policies": active_policies, "traces": traces}

    def get_settings(self) -> PolicySettings:
        if self.db:
            try:
                doc = self.db.collection('settings').document('global').get()
                if doc.exists: return PolicySettings(**doc.to_dict())
            except: pass
        return PolicySettings()

    def save_settings(self, settings: PolicySettings):
        if self.db:
            try: self.db.collection('settings').document('global').set(settings.model_dump())
            except Exception as e: print(f"Failed to save settings to Firebase: {e}")
